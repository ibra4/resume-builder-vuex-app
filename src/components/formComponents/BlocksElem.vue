    ...
    <div>
        <div class="row">
            <div v-for="sk in obj" :key="sk.id" class="col-md-6">
                <text-field :targetElem="objName + '|title|' + sk.id" :name="'Block ' + (sk.id + 1)"/>
                <button v-if="obj.length > 1" @click="deleteObj(sk.id)" class="btn btn-danger" style="float: right">delete {{sk.id}} </button>
            </div>
        </div>
        <button v-if="obj[Object.keys(obj).length-1].title != ''" @click="addObj()" class="btn btn-primary">add new object </button>
    </div>
</template>
